...
public:
	...
	~SdfRenderer();
	//отсутствует документация к этим функциям
...
private:
	...
	class ImagesCache { //разное оформление фигурных скобок (здесь и далее)
	public:
	...
	int mStartX;
	int mStartY;
	//Странное название для полей, если это счётчики не несущеие смысла - то зачем поля, иначе - нужно выразить смысл в названии
	int i;
	int j;
	int sep;
	QPainter *painter;
	QPen pen;
	QBrush brush;
	QString s1;//не самые удачные названия для строк
	QString s2;
	QFont font;
	QFile log;
	QTextStream logtext;
	...
	**/
	...
	QPoint *getpoints(QDomElement &element, int n);//Пропал CamelCase
...
//Несколько классов в одном файле - не всегда хорошо
