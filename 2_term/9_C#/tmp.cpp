...
bool SdfRenderer::load(const QString &filename)
{
	...
	//CamelCase соседствует с подчёркиваниями (здесь и далее)
...
//Различается стиль расстановки фигурных скобок (здесь и далее)
...
void SdfRenderer::render(QPainter *painter, const QRectF &bounds, bool isIcon)
{
	...
	while(!node.isNull())
	{
		...
		if(!elem.isNull())
		{
			...
			{
				//Тоже самое про CamelCase и подчёркивания (здесь и далее)
				draw_text(elem);
			}
...
void SdfRenderer::line(QDomElement &element)
{
	...
	//лучше parseStyle - CamelCase (здесь и далее)
	parsestyle(element);
...
void SdfRenderer::background(QDomElement &element)
{
	...
	//Лучше defaultStyle - CamelCase (здесь и далее)
	defaultstyle();
}
...
void SdfRenderer::image_draw(QDomElement &element)
{
	...
	//Венгерская нотация? Стоит наверное применять её либо везде, либо нигде (здесь и далее)
...
{
	QPointF end_point;
	//Не совсем ясные названия с1 и с2
	QPointF c1;
	QPointF c2;
	QDomElement elem = element;
	QPainterPath path;
...
void SdfRenderer::ImagesCache::drawImage(
		...
		, QPainter &painter
		, const QRect &rect)//запятые лучше оставлять на конце строки
{
	auto savePrerenderedSvg = [this, &rect, &fileName] (QSvgRenderer &renderer) { //опять разное оформление фигурных скобок (здесь и далее)
		QTransform scale;
...
void SdfIconEngineV2::paint(QPainter *painter, const QRect &rect, QIcon::Mode mode, QIcon::State state)
{
	...
	if (pw == 0 || ph == 0) { // SUDDENLY!!11 //Главное чтобы тестовый вывод с таким не оставляли в продакшене
		return;
	}
